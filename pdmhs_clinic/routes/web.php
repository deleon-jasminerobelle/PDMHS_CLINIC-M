...
|--------------------------------------------------------------------------
*/
Route::middleware(['auth', 'check.health.form'])->prefix('student')->name('student.')->group(function () {
...
Route::middleware(['auth'])->prefix('clinic-staff')->name('clinic-staff.')->group(function () {
    ...
    Route::get('/students/search', [ClinicStaffController::class, 'searchStudents'])->name('students.search');
...
Route::middleware(['auth'])->group(function () {
    ...
    // Student health form
    Route::get('/student-health-form', fn() => view('student-health-form'))->name('student-health-form');
...
Route::post('/keep-alive', fn() => Auth::check() ? response()->json(['status'=>'alive','user'=>Auth::user(),'timestamp'=>now()->toISOString()]) : response()->json(['status'=>'unauthenticated'],401))->middleware('auth');
Route::get('/session-status', fn() => response()->json(['authenticated'=>Auth::check(),'user'=>Auth::user(),'session_id'=>session()->getId(),'csrf_token'=>csrf_token(),'timestamp'=>now()->toISOString()]));
...
Route::get('/create-test-users', function () {
    ...
    return response()->json(['message'=>'Test users created successfully!','users'=>$users]);
});
